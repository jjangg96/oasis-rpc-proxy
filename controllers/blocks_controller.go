...
type GetBlockPrecommitsResponse struct {
	Message string       `json:"message"`
	Data    []*types.CommitSig `json:"data"`
}
...
func GetBlockHeader(c *gin.Context) {
	...
	c.JSON(http.StatusOK, GetBlockHeaderResponse{Message: "Success", Data: *tmBlockMeta.Header})
}

func GetBlockPrecommits(c *gin.Context) {
	height, err := (strconv.ParseInt(c.Param("height"), 10, 64))
	if err != nil {
		log.Error("height must be a number", err)
		c.JSON(http.StatusBadRequest, utils.ApiError{Message: "height must be a number"})
	...
	c.JSON(http.StatusOK, GetBlockPrecommitsResponse{Message: "Success", Data: tmBlockMeta.LastCommit.Precommits})
}
